...
public class MyPageController {
	...
	public String myContent(@RequestParam(value = "page", defaultValue = "1") int page,													 
		...
		/* 최근 포스팅 일자 조회 */
		String myContentDate = mypageService.myContentDate(user_idx);
	...
	public String myScrap(@RequestParam(value = "page", defaultValue = "1") int page,
		...
		/* 내가 올린 게시글 미리보기 조회 */
		List<ContentDTO> myScrapList = mypageService.myScrapPreview(user_idx, page);
		model.addAttribute("myScrapList", myScrapList);	
		
		/* 페이지네이션을 위한 PageDTO 선언 */
		PageDTO pageDTO = mypageService.getCountOfMyScrapTotal(user_idx, page);
		model.addAttribute("pageDTO", pageDTO);		
