...
public interface CommentMapper {
    ...
    @Select("SELECT C.COMMENT_IDX, C.USER_IDX, C.PARENT_COMMENT_IDX, C.COMMENT_TEXT, " +
        ...
        "C.IS_DELETED, C.IS_HIDDEN, C.UPVOTE_COUNT, C.DOWNVOTE_COUNT, " +
        "U.USER_NICKNAME, V.VOTE_TYPE, V.VOTE_IDX " +
        "FROM COMMENT_TABLE C " +
        "LEFT JOIN USER_TABLE U ON C.USER_IDX = U.USER_IDX " +
        "LEFT JOIN COMMENT_VOTE_TABLE V ON C.COMMENT_IDX = V.COMMENT_IDX " +
    ...
    void insertReply(@Param("user_idx") Long userIdx, 
                     @Param("parent_comment_idx") Long parentCommentIdx, 
                     @Param("comment_text") String commentText, 
                     @Param("post_id") Long postId,
                     @Param("comment_level") int commentLevel);
    ...
    void insertVote(@Param("comment_idx") Long commentIdx, @Param("user_idx") Long userIdx, @Param("vote_type") Integer voteType);

    // 댓글 투표 업데이트
    @Update("UPDATE COMMENT_VOTE_TABLE SET VOTE_TYPE = #{vote_type} " +
            "WHERE COMMENT_IDX = #{comment_idx} AND USER_IDX = #{user_idx}")
    void updateVote(@Param("comment_idx") Long commentIdx, @Param("user_idx") Long userIdx, @Param("vote_type") Integer voteType);

    // 특정 댓글에 대한 사용자의 투표 여부와 타입 확인
    @Select("SELECT VOTE_TYPE FROM COMMENT_VOTE_TABLE " +
            "WHERE COMMENT_IDX = #{comment_idx} AND USER_IDX = #{user_idx}")
    String checkVoteType(@Param("comment_idx") Long commentIdx, @Param("user_idx") Long userIdx);
